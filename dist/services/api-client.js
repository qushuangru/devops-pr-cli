...
class AzureDevOpsClient {
    constructor(config, repoInfo) {
        ...
        if (!organization || !project) {
            throw new Error('Organization and project must be detected from git remote or provided in config.\n' +
    ...
    async createPR(params) {
        try {
            ...
            const response = await this.axiosInstance.post(`/git/repositories/${params.repositoryId}/pullrequests?api-version=6.0`, payload);
    ...
    async listPRs(params) {
        try {
            ...
            const response = await this.axiosInstance.get(`/git/repositories/${params.repositoryId}/pullrequests?${queryParams.toString()}`);
    ...
    async getPR(repositoryId, pullRequestId) {
        try {
            const response = await this.axiosInstance.get(`/git/repositories/${repositoryId}/pullrequests/${pullRequestId}?api-version=6.0`);
            return response.data;
        }
        catch (error) {
            throw this.handleError(error);
    ...
    async getPRThreads(repositoryId, pullRequestId) {
        try {
            const response = await this.axiosInstance.get(`/git/repositories/${repositoryId}/pullrequests/${pullRequestId}/threads?api-version=6.0`);
            return response.data.value || [];
        }
        catch (error) {
            throw this.handleError(error);
    ...
    async getRepositoryId(repositoryName) {
        try {
            const response = await this.axiosInstance.get(`/git/repositories/${repositoryName}?api-version=6.0`);
    ...
    handleError(error) {
        if (axios_1.default.isAxiosError(error)) {
            ...
            if (status === 401) {
                return new Error('Authentication failed. Your PAT token may be invalid or expired.\n' +
                    'Run "devops-pr config init" to reconfigure.');
            }
            if (status === 403) {
                return new Error('Permission denied. Your PAT token does not have sufficient permissions.\n' +
                    'Ensure your token has "Code (Read)" and "Pull Requests (Read & Write)" permissions.');
            }
            if (status === 404) {
                return new Error('Resource not found. The repository or PR may not exist.\n' +
                    'Check your repository name and PR ID.');
            }
            if (axiosError.code === 'ECONNREFUSED' || axiosError.code === 'ENOTFOUND') {
                return new Error(`Unable to connect to ${this.server}\n` +
